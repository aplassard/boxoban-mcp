...
def test_load_game_from_string_box_on_target():
    ...
    # Player is missing here. This should fail init or parsing.
    with pytest.raises(ValueError, match="Player .* not found"):
        # This file would represent "####\n#*.#\n####" which has no player
        BoxobanGame.load_game_from_file("tests/data/box_on_target_no_player_error.txt") # Assuming this file exists for this test case
...
def temp_puzzle_file(tmp_path):
    # file_content = "0\\n####\\n#@.#\\n####;1\\n#####\\n#@$.#\\n#####" # Original content
    # filepath = tmp_path / "test_puzzles.txt" # Original dummy file
    # create_dummy_puzzle_file(filepath, file_content) # Was creating dummy
    return "tests/data/multi_puzzle_test.txt" # Now provides path to the actual test file
...
def test_load_game_from_file_single_raw(tmp_path):
    # file_content = "####\\n#@.#\\n####" # Raw puzzle, no index/header
    # filepath = tmp_path / "single_raw.txt" # Original dummy file
    # create_dummy_puzzle_file(filepath, file_content) # Was creating dummy
    game = BoxobanGame.load_game_from_file("tests/data/simple_board.txt", puzzle_index=0) # puzzle_index still needed for raw
    assert game.get_game_state() == "####\n#@.#\n####"

def test_load_game_from_file_single_with_header(tmp_path):
    # file_content = "0\\n####\\n#@.#\\n####" # Puzzle with index 0 header
    # filepath = tmp_path / "single_with_header.txt" # Original dummy file
    # create_dummy_puzzle_file(filepath, file_content) # Was creating dummy
...
def temp_puzzle_structure(tmp_path):
    ...
    # Read board content from the actual file
    with open("tests/data/board_for_params.txt", 'r') as f:
        board_content = f.read().strip() # Ensure no trailing newlines if not part of board

    file_content = f"12\n{board_content}" # Construct file content with header and board
    filepath = full_dir / f"{puzzle_set_num_str}.txt"
    with open(filepath, 'w') as f:
        f.write(file_content)
...
def test_load_game_from_params(temp_puzzle_structure):
    original_cwd = os.getcwd()
    os.chdir(temp_puzzle_structure)

    # Read expected board content for assertion
    with open(os.path.join(original_cwd, "tests/data/board_for_params.txt"), 'r') as f:
        expected_board_content = f.read().strip()
...
def test_get_valid_moves_simple():
    game = BoxobanGame.load_game_from_file("tests/data/valid_moves_simple_1.txt")
    assert sorted(game.get_valid_moves()) == sorted(['right'])

    game_open = BoxobanGame.load_game_from_file("tests/data/valid_moves_simple_game_open.txt")
    assert sorted(game_open.get_valid_moves()) == sorted(['up', 'down', 'left', 'right'])

def test_get_valid_moves_wall():
    game = BoxobanGame.load_game_from_file("tests/data/valid_moves_wall.txt")
    assert game.get_valid_moves() == []

def test_get_valid_moves_push_box_empty_behind():
    game = BoxobanGame.load_game_from_file("tests/data/valid_moves_push_empty_1.txt")
    assert game.get_valid_moves() == ['right']

    game_left = BoxobanGame.load_game_from_file("tests/data/valid_moves_push_empty_left.txt")
    assert game_left.get_valid_moves() == ['left']


def test_get_valid_moves_push_box_wall_behind():
    game = BoxobanGame.load_game_from_file("tests/data/valid_moves_push_wall_1.txt")
    assert game.get_valid_moves() == []

    game_blocked_both_sides = BoxobanGame.load_game_from_file("tests/data/valid_moves_push_wall_blocked_sides.txt")
    # Note: The original test for game_blocked_both_sides asserted on `game`, not `game_blocked_both_sides`.
    # Assuming it was a typo and should be `game_blocked_both_sides.get_valid_moves()`.
...
def test_is_solved():
    game1 = BoxobanGame.load_game_from_file("tests/data/is_solved_game1.txt") # Was "####\\n#@*#\\n####"
    assert game1.is_solved() is True

    game2 = BoxobanGame.load_game_from_file("tests/data/is_solved_game2.txt") # Was "####\\n#+*#\\n####"
    assert game2.is_solved() is False # As per original logic

    # game3_str = "#*#\\n#*#" # No player, should fail
    with pytest.raises(ValueError):
        BoxobanGame.load_game_from_file("tests/data/is_solved_game3_no_player_error.txt") # Assuming this file exists

    game3 = BoxobanGame.load_game_from_file("tests/data/is_solved_game3_ok.txt") # Was "#*@\\n#*#"
    assert game3.is_solved() is True
...
def test_is_not_solved():
    ...
    game2 = BoxobanGame.load_game_from_file("tests/data/simple_board.txt") # Was "####\\n#@ .#\\n####" (using simple_board as it's P @ T .)
    assert game2.is_solved() is False

    game3_with_box = BoxobanGame.load_game_from_file("tests/data/is_not_solved_game3_with_box.txt") # Was "#+$ .#"
    assert game3_with_box.is_solved() is False

    game4 = BoxobanGame.load_game_from_file("tests/data/solvable_setup.txt") # Was "#@$ .#"
    assert game4.is_solved() is False
...
class TestDeadlockDetection(unittest.TestCase):
    def test_corner_deadlock_box_not_on_goal(self):
        game = BoxobanGame.load_game_from_file("tests/data/dl_corner_box_not_on_goal.txt")
        self.assertTrue(game._is_deadlock(game.board))

    def test_corner_deadlock_box_on_goal(self):
        game = BoxobanGame.load_game_from_file("tests/data/dl_corner_box_on_goal.txt")
    ...
    def test_frozen_vertical_north_wall_no_escape(self):
        game = BoxobanGame.load_game_from_file("tests/data/dl_frozen_vert_no_escape.txt")
        self.assertTrue(game._is_deadlock(game.board))

    def test_frozen_vertical_north_wall_escape_left(self):
        game = BoxobanGame.load_game_from_file("tests/data/dl_frozen_vert_escape_left.txt")
        self.assertFalse(game._is_deadlock(game.board))

    def test_frozen_vertical_north_wall_escape_right_path_blocked_by_box(self):
        game = BoxobanGame.load_game_from_file("tests/data/dl_frozen_vert_escape_right_box.txt")
        self.assertTrue(game._is_deadlock(game.board))

    def test_frozen_horizontal_west_wall_no_escape(self):
        game = BoxobanGame.load_game_from_file("tests/data/dl_frozen_horiz_no_escape.txt")
        self.assertTrue(game._is_deadlock(game.board))

    # --- 2x2 Box Deadlocks ---
    def test_2x2_box_deadlock_no_goals(self):
        game = BoxobanGame.load_game_from_file("tests/data/dl_2x2_no_goals.txt")
        self.assertTrue(game._is_deadlock(game.board))

    def test_2x2_box_deadlock_all_goals(self):
        game = BoxobanGame.load_game_from_file("tests/data/dl_2x2_all_goals.txt")
        self.assertFalse(game._is_deadlock(game.board))

    def test_2x2_box_deadlock_some_goals(self):
        # Original board_string = "#####\\n#*$@#\n#$$ #\n# ..#\n#####"
        game_orig_some_goals = BoxobanGame.load_game_from_file("tests/data/dl_2x2_some_goals_1.txt")
        ...
        # Original board_2x2_TL_not_goal = "#####\\n#$$@#\n#** #\n# . #\n#####"
        game_tl_not_goal = BoxobanGame.load_game_from_file("tests/data/dl_2x2_some_goals_2.txt")
    ...
    def test_gvm_prevents_corner_deadlock(self):
        game = BoxobanGame.load_game_from_file("tests/data/gvm_prevents_corner_deadlock.txt")
        self.assertNotIn('right', game.get_valid_moves())

    def test_gvm_allows_safe_push_to_goal(self):
        game = BoxobanGame.load_game_from_file("tests/data/gvm_allows_safe_push_to_goal.txt")
